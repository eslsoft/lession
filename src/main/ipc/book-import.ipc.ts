...
export function registerBookImportIpc(): void {
  ...
  ipcMain.handle(IPC.BOOK_IMPORT_PREVIEW, async (_event, serviceId: string, voice: string, speed: number, text?: string) => {
    ...
    const providerType = service.providerType
    const ext = (providerType === 'kokoro') ? '.wav' : '.mp3'
    const outputPath = path.join(app.getPath('temp'), `tts-preview-${Date.now()}${ext}`)
    return previewTts(serviceId, voice, speed, previewText, outputPath)
  })

  ipcMain.handle(IPC.BOOK_IMPORT_GENERATE, (_event, seriesId: string, epubPath: string, chapters: { title: string; text: string }[], serviceId: string, voice: string, speed: number) => {
    return startBookImport(seriesId, epubPath, chapters, serviceId, voice, speed)
  })
